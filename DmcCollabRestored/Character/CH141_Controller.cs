    ...
    public class CH141_Controller : CharacterControlBase, ITangerineLogicUpdate
    {
        ...
        public override void PlayerPressSkillCharacterCall(int id)
        {
            ...
            else
            {
                ...
                Tangerine.Utils.Il2CppHelpers.FxManagerPlay(this.FX_0_00, this._refEntity.ModelTransform.position, OrangeCharacter.NormalQuaternion);
        ...
        private void UseSkill1()
        {
            ...
            else
            {
                ...
                if (aimTarget == null)
                {
                    ...
                    {
                        vector = new Vector3(this._refEntity.AimPosition.x, this._refEntity.transform.position.y, this._refEntity.AimPosition.z);
                    }
                    else
                    {
                        vector = this._refEntity.AimPosition;
                    }
                    vector.x += Mathf.Sign(this._refEntity.ShootDirection.x) * f_DISTANCE;
                }
                else
                {
                    vector = aimTarget.AimPosition;
                    if (this.IsSkl1InGround && Mathf.Abs(vector.y - this._refEntity.AimPosition.y) > 0.2f)
                    {
                        this.IsSkl1InGround = false;
                    }
                }
                this._targetPos = new Vector3?(Vector3.MoveTowards(this._refEntity.AimPosition, vector, f_DISTANCE));
            }
                        ...
                        case OrangeCharacter.SubStatus.SKILL1_1:
                            {
                                if (this._targetPos != null)
                                {
                                    Vector3 vector = this._targetPos.Value - this._refEntity.AimPosition;
        ...
        {
            return new Il2CppStringArray(new string[] { "ch141_skill_01_crouch", "ch141_skill_01_stand", "ch141_skill_01_jump", "ch141_skill_02_stand_start", "ch141_skill_02_stand_loop", "ch141_skill_02_stand_end", "ch141_skill_02_jump_start", "ch141_skill_02_jump_loop", "ch141_skill_02_jump_end" });
        }
